    ...
    stages {
        ...
        stage('Deploy to Ansible Server') {
            steps {
                ...
                withCredentials([sshUserPrivateKey(credentialsId: '2a67236b-f587-44a4-90f2-1e8ba1e8d313', keyFileVariable: 'SSH_PRIVATE_KEY')]) {
                    sh '''
                    rsync -e "ssh -o StrictHostKeyChecking=no -i $SSH_PRIVATE_KEY" -av --exclude venv/ ./ ec2-user@ec2-34-253-150-156.eu-west-1.compute.amazonaws.com:/home/ec2-user/ansible-data/
                    '''
                }
            }
        }
        ...
        stage('Deploy using Ansible') {
            steps {
                ...
                withCredentials([sshUserPrivateKey(credentialsId: '2a67236b-f587-44a4-90f2-1e8ba1e8d313', keyFileVariable: 'SSH_PRIVATE_KEY')]) {
                    sh '''
                    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$SSH_PRIVATE_KEY" ec2-user@ec2-34-253-150-156.eu-west-1.compute.amazonaws.com 'ansible-playbook -i /etc/ansible/hosts /home/ec2-user/ansible-data/ansible-playbooks/deploy-web-app.yml'
                    '''
                }
            }
        }
